...
st.set_page_config(page_title="Projects — Founder Revenue OS", page_icon="📁", layout="wide")
inject_css()
                ...
                with ec1:
                    with st.form(f"edit_form_{row['project_id']}"):
                        ...
                        new_status = st.selectbox(
                            ...
                            index=project_statuses.index(row["project_status"]) if row["project_status"] in project_statuses else 0,
                        ...
                        if st.form_submit_button("Save Changes"):
                            sheets.update_row(
                                ...
                                {"project_status": new_status, "notes": new_notes},
                            )
                            st.toast("Project updated", icon="✅")
                            st.rerun()
                with ec2:
                    st.markdown("**Danger Zone**")
                    if st.button("🗑️ Delete Project", key=f"del_{row['project_id']}"):
